...
if (!class_exists("CommunityViewGroups")) {
    ...
    class CommunityViewGroups extends CommunityView {
               ...
               // $this->addSubmenuItem('index.php?option=com_community&view=groups&task=viewmembers&groupid=' . $groupid, JText::_('COM_COMMUNITY_GROUPS_ALL_MEMBERS'));
                ...
                if ($task == 'viewmembers' && !$isBanned) {
                    ...
                    if (COwnerHelper::isRegisteredUser()) {
						...
						$this->addSubmenuItem('index.php?option=com_community&view=groups&task=mymembership&userid=' . $my->id, JText::_('My Memberships'));
                    }
		...
		public function mymembership() {
             ...
             * Opengraph
             */
            CHeadHelper::setType('website', JText::_('My Memberships'));

            $my = CFactory::getUser();
            $model = CFactory::getModel('groups');
            
            $rows = $model->getGroupMembership($my->id);

            $tmpl = new CTemplate();
            echo $tmpl->set('membership', $rows)
                    ->set('my', $my)
                    ->set('submenu', $this->showSubmenu(false))
