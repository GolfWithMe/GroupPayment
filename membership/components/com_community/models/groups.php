...
// 		}
	...
	**/
	...
	public function addGroupMembership($groupid, $userid, $resArray, $membership)
    {
        ...
        $query	= 'INSERT INTO '. $db->quoteName('#__community_groups_membership')
	...
	public function addJoinGroup($groupid, $userid)
    {
		...
		//invite
		$query	= 'DELETE FROM '. $db->quoteName('#__community_groups_invite')
            .' WHERE ' . $db->quoteName('groupid').' = '.$db->Quote($groupid)
            . ' AND '. $db->quoteName('userid').' = '.$db->Quote($userid);
        ...
        $query	= 'INSERT INTO '. $db->quoteName('#__community_groups_members')
            ...
            . ', '. $db->quoteName('approved').' = '. $db->Quote(1)
			. ', '. $db->quoteName('permissions').' = '. $db->Quote(0);
